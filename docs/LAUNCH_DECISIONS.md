...
| Most recent application release baseline (historical evidence gaps disclosed) | Pinned-GitHub-Actions release commit `3e31933189cb74df8b1565dadbb3419375c4f6f8`; GitHub/Vercel Production deployment `5553345433` / `F4NmkqsGjWJdRCCQ7gNveLqkumxP`; Production Quality run `29909912133` and production smoke run `29909963039` passed; authenticated alias evidence is `docs/evidence/vercel-production-alias-2026-07-22-pr34.json`. `docs/releases/2026-07-22-pr34.md` records the exact candidate, Preview, Production, READY-only canonical alias transition, reviewed full-SHA action pins, Chromium/Firefox/desktop and mobile WebKit, seven visual baselines, five-route Lighthouse budgets, private/public demo boundary, and zero-failure public smoke while disclosing the protected-Preview semantic-smoke, current authenticated-playback, no-JavaScript mobile-navigation, accessibility-landmark/focus, and custody gaps. Each later release must retain its own PR/Vercel evidence rather than silently overwriting this baseline. | Engineering release / 2026-07-22 |
