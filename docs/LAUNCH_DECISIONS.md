...
| Review start and absolute expiry time | Configured for Production only; all dynamic Preview deployments fail closed. Production closes `2026-07-28T15:59:00Z` (`2026-07-28 23:59` China Standard Time). `config/executive-preview-window.json` enforces that timestamp as the maximum Production value, while allowing an earlier safe close; the credential-free scheduled lifecycle smoke checks the boundary every six hours. | Engineering configuration and repository enforcement / 2026-07-22 |
...
| Most recent application release baseline (historical evidence gaps disclosed) | Performance/quality release commit `e5a18964c5cba2035e1221314b0f29ec4eb93089`; GitHub/Vercel Production deployment `5547952436` / `EDKwCDxC2zm4E5X51kMWKMU8HgyE`; Production Quality run `29880825422` and production smoke run `29880861964` passed; authenticated alias evidence is `docs/evidence/vercel-production-alias-2026-07-22-pr20.json`. `docs/releases/2026-07-22-pr20.md` records the exact candidate, Preview, Production, canonical alias assignment, lifecycle gates, private/public demo boundary, performance diagnostics, and public smoke evidence while disclosing the protected-Preview semantic-smoke and current authenticated-playback gaps. Each later release must retain its own PR/Vercel evidence rather than silently overwriting this baseline. | Engineering release / 2026-07-22 |
