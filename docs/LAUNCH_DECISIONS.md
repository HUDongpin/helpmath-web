...
| Confirm session maximum is 12 hours and bounded by the global expiry | Implemented and covered by unit and browser checks. A current-commit, owner-authenticated Production operator check reported all `4/4` localized private demo routes, `12/12` private images, `2/2` private runtimes, `failures: []`, a focused two-demo browser result of `1/1`, and the observed global close `2026-07-28T15:59:00.000Z`. The reduced non-secret operator attestation is `docs/evidence/executive-preview-production-2026-07-22.json`, SHA-256 `541a6c7cb0da495f18dd271a8fdcd311291f88d8abf3c211050683c9890abc4b`. No canonical machine result, raw output, access value, signing value, cookie, trace, screenshot, or browser state was retained, so the authenticated counts are not independently reproducible. Private delivery, recipient acknowledgement, and the post-meeting close action remain pending. | Engineering operator attestation / 2026-07-22 |
...
| Most recent application release baseline (historical evidence gaps disclosed) | Launch-gate release commit `05c3b460db78ac61b88b6d34479d925af634d724`; GitHub Production deployment `5543929832`; current Vercel deployment `dpl_2CCekiWMGdnwc78gdifVSs1NWNsX`; Production Quality run `29858878638`, zero-failure production-smoke run `29859546466`, and same-commit `16/16` stable-link run `29860501241` passed. `docs/releases/2026-07-22-pr13.md` records the exact candidate, protected Preview, Production, current canonical alias assignment, launch-gate state, operator-attested authenticated private-demo check, stable links, and scoped home-page UI evidence. Supporting records are `docs/evidence/vercel-production-alias-2026-07-22.json`, `docs/evidence/executive-preview-production-2026-07-22.json`, and `docs/evidence/production-ui-audit-2026-07-22.json`. The protected-Preview semantic-smoke and external approval gaps remain disclosed, every launch gate remains `holding`, and the final cutover candidate still requires fresh exact-commit preflight evidence before DNS changes. | Engineering release / 2026-07-22 |
