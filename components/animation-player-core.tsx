...
export function AnimationPlayerCore({
  animation,
  content,
  demoId,
  locale,
  ...
  const [playbackIntent, setPlaybackIntent] = useState<'auto' | 'playing' | 'paused'>(
    startPaused ? 'paused' : 'auto',
  );
  const liveFrameRef = useRef(1);
  const isPlaying = playbackIntent === 'playing' || (playbackIntent === 'auto' && !reduceMotion);

  useEffect(() => {
    if (requestedFrame || !isPlaying) return;
